...
#     two metropolis algorithm starting from the same initial lattice
    ...
    for i in range(timesteps + t_to_eq):
        ...
        if i >= t_to_eq:
            energies[i] = (system_energy(lattice, coupling_constant, magn_field)/size**2)
            magnetizations[i] = (np.sum(lattice)/size**2)
...
def average_chi(size, coupling_constant, temperature, timesteps, magn_field): #test
    ...
    for i in range(iterations):
        energies, magnetizations = ising_simulation(size, coupling_constant, temperature, timesteps, magn_field, first_poss, second_poss, third_poss, fourth_poss, fifth_poss)
...
def run_simulation():
    ...
    coupling_constant = 1
    n_steps = 100000
    temperature = np.arange(1.0, 1.1, 0.2)
    magn_field = 0
    ...
    for i in range(len(temperature)):
        correlation_time[i], correlation_time_error = average_tau(n_spins, coupling_constant, temperature[i], n_steps, magn_field)
